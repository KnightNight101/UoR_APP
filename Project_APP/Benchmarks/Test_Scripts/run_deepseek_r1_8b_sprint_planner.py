    ...
    for t in range(n_tasks):
        ...
        if random.random() < 0.3:
            for s in range(random.randint(1,2)):
                tasks.append({"id": next_id, "title": f"Task_{t}_sub{s}", "do": max(1, do_h//2), "verify": max(1, verify_h//2), "dependencies": [], "parent": t})
                next_id += 1
    # Add dependencies
    for t in tasks:
...
def parse_model_schedule(output: str) -> Dict[int, Dict[str, Any]]:
    ...
    for item in backlog:
        try:
            ...
            finish=start+dur
            result[tid]={"task_id":tid,"assignee":assignee,"start":start,"duration":dur,"finish":finish}
        except: continue
    return result
...
def repair_model_schedule(case: Dict[str, Any], model_sched: Dict[int, Dict[str, Any]]) -> Dict[int, Dict[str, Any]]:
    ...
    for tid,task in sorted(model_sched.items(), key=lambda x:x[1]["start"]):
        ...
        earliest_start=max([repaired[d]["finish"] for d in deps], default=0.0)
        ...
        repaired[tid]={"task_id":tid,"assignee":assignee,"start":start_time,"duration":duration,"finish":finish_time}
...
def evaluate_model_schedule(case: Dict[str, Any], model_sched: Dict[int, Dict[str, Any]], baseline_makespan: float) -> Dict[str, Any]:
    ...
    return {"baseline_makespan":baseline_makespan,"model_makespan":model_makespan,"makespan_ratio":round(makespan_ratio,3),
            "dependency_violations":violations,"task_coverage":round(task_coverage,3),
            "avg_member_utilization":round(avg_util,2),"feasible":feasible}
...
# --------------------------
def main():
    headers=["test_id","model","baseline_makespan","model_makespan","makespan_ratio",
             "dependency_violations","task_coverage","avg_member_utilization","feasible","time_s"]
    ...
    for test_id in range(1,NUM_TESTS+1):
        ...
        row=[test_id,MODEL_NAME,metrics["baseline_makespan"],metrics["model_makespan"],metrics["makespan_ratio"],
             metrics["dependency_violations"],metrics["task_coverage"],metrics["avg_member_utilization"],
             metrics["feasible"],round(elapsed,3)]
